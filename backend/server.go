...
func (s *Server) handleCreateGame(w http.ResponseWriter, r *http.Request) {
	...
	respondWithJSON(w, http.StatusOK, map[string]string{"gameId": gameID})
}
...
func (s *Server) handleJoinGame(w http.ResponseWriter, r *http.Request) {
	...
	// Set first player as current player if no current player is set
	if game.Board.CurrentPlayer == "" && len(game.Board.Players) > 0 {
		game.Board.CurrentPlayer = game.Board.Players[0].Username
	}

	// send propeties to the player
	propertiesMsg := PropertiesMessage{
		Places: Places,
	}

	// send properties to the player
	if err := conn.WriteJSON(propertiesMsg); err != nil {
		log.Println("Error writing properties message to player:", err)
		return
	}

	// send competitor and monopolist cards to the player
	competitorCardsMsg := CardsMessage{
		Cards: game.Board.CompetitorCards,
	}
	monopolistCardsMsg := CardsMessage{
		Cards: game.Board.MonopolistCards,
	}
	if err := conn.WriteJSON(competitorCardsMsg); err != nil {
		log.Println("Error writing competitor cards message to player:", err)
		return
	}
	if err := conn.WriteJSON(monopolistCardsMsg); err != nil {
		log.Println("Error writing monopolist cards message to player:", err)
		return
	}
